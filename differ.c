...
int para_almostequal(para* p, para* q, int* lines_equal, int nlines) {
  ...
  memset(lines_equal, 1, nlines * sizeof(int)); // SET ALL LINES TO BE EQUAL BY DEFAULT
...
void init_options_files(int argc, const char* argv[]) {
  ...
  if(report_identical && showbrief && different){ printf("The files are different.\n"); exit(0); } //for when both s and q are on...
  if(report_identical && showbrief && !different){ printf("The files are identical.\n"); exit(0); }

  if (showbrief && different) { printf("The files are different.\n\n");   exit(0); } //if q and the files are different
  if (showbrief && !different){ exit(0); } //if q and the files are the same dont respond

  if (report_identical && !different) { printf("The files are identical.\n\n");   exit(0); } //if s and the files are the same
}
...
int main(int argc, const char * argv[]) {
  ...
  while (p != NULL) {
    ...
    if (foundmatch) {
      while ((foundmatch = para_equal(p, q)) == 0) { //if they are not equal it will print on the right
        if(diffnormal){
          norm(p, q);
          para_print(q, printrightonly);
          q = para_next(q);
          qlast = q;
          suppresscommon =1;
        }else{
      ...
      if(!suppresscommon){
        if(showleftcolumn){
          para_print(q, onlyleft); //PRINTS COMMON LINES TO THE LEFT ONLY
          p = para_next(p);
          q = para_next(q);
        }else{ // PRINTS OUT BOTH THE COMMMON PARAGRAPHS
          para_print(q, printboth);
          p = para_next(p);
          q = para_next(q);
        }
      }
     q = para_next(q);//This will skip the paragraps that are being supressed
     p = para_next(p);              
    } else {
      if(diffnormal){
        norm(p,q);
